...
/**
  * This is a heuristic scheduler that linearly  backs off when no AP is available.
  *
  * @author     Hossein Falaki
  */
public class LBScheduler implements Scheduler, Logger
{
    ...
    /** The linear step of back off time */
    double backoffstep;

    /** The start back-off value */
    ...
    /**
      ...
      * @param      backoffstep         value added to the back-off time on every step
      * @param      initialbackoff      the initial value for back-off
      * @param      randomize           indicates if the process should be randomized
      */
    public LBScheduler(double endtime, double backoffstep, double initialbackoff, WiFiNIC nic, boolean randomize)
    {
        now = 0.0;
    ...
    /**
      ...
      * LBScheduler does not send any commands beyond end time.
      * 
    ...
    /**
      ...
      * LBScheduler is stateless and its behavior does not depend on the user,
    ...
    /**
      ...
      * This scheduler follows a linear  back off algorithm to respond to queries.
    ...
    public ArrayList query(ArrayList context) throws SchedulerException
    {
        ...
        if (niccondition == WiFiNIC.DISC_SCANNING )
        {
            ...
            if ( ap == null)
            {
                ...
                Log.debug(this, "Backing off until " + backofftime + " backoff: " + backoff );
                command.add(WiFiNIC.OFF);
    ...
    private double nextbackofftime()
    {
        double backofftime = 0;
        if (randomize)
            backofftime = now + ((Math.random() * backoffstep) + (backoff - backoffstep));
        else
            backoff = now + backoff;
